...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    match app.mode {
        Mode::Insert | Mode::Normal => {
             let chunks = Layout::default()
                ...
                .constraints([
                    ...
                    Constraint::Length((3 + app.input.lines().len().saturating_sub(1) as u16).min(10)), // Input grows to max 10 lines
                ])
                .split(size);
            
            // Header
            let model_name = app.models.get(app.selected_model).map(|s| s.as_str()).unwrap_or("No Model");
            ...
            let title = if let Some((status, completed, total)) = &app.pull_progress {
                 ...
                 format!(" Pulling Model: {} ({}%) - {} (F1 for Help) ", status, percent, app.current_session)
            } else {
                 format!(" Ollama TUI - {} - Session: {} (F1 for Help) ", model_name, app.current_session)
            };

            let header_block = Block::default().borders(Borders::ALL).title(title).style(Style::default().fg(Color::Cyan)).border_type(BorderType::Rounded);
            ...
            for (i, (height, text_opt, is_thinking)) in calculated_msgs.into_iter().enumerate() {
                ...
                if current_y + (bubble_height as i32) > 0 && current_y < (viewport_height as i32) {
                     ...
                     let x = if is_user { width.saturating_sub(bubble_width) } else { 0 };
                     ...
                     if visible_bottom > visible_top {
                         ...
                         let (border_color, title) = if is_user { (Color::Green, " You ") } else { (Color::Cyan, " AI ") };
                         ...
                         if is_thinking {
                             let throbber = Throbber::default()
                                .label("Thinking...")
                                .throbber_style(Style::default().fg(Color::LightCyan).add_modifier(Modifier::BOLD));
                             ...
                             let scroll_offset = if item_top < area_top as i32 { (area_top as i32 - item_top) as u16 } else { 0 };
            ...
            let (input_border_color, input_title) = if let Some(err) = &app.error {
                ...
                match app.mode {
                    Mode::Insert => (Color::Green, " Input (Insert Mode) - Esc for Normal ".to_string()),
                    Mode::Normal => (Color::Blue, " Input (Normal Mode) - i to Type ".to_string()),
                    Mode::SystemPromptEdit => (Color::Yellow, " Edit System Prompt (Esc to Cancel, Enter to Save) ".to_string()),
            ...
            match app.mode {
                ...
                _ => app.input.set_style(Style::default().add_modifier(Modifier::DIM)),
            }
        ...
        Mode::ModelSelect => {
             ...
             let block = Block::default().title(" Select Model (p: Pull, d: Delete) ").borders(Borders::ALL).border_type(BorderType::Rounded);
             let items: Vec<ListItem> = app.models.iter().enumerate().map(|(i, m)| {
                 let s = if i == app.selected_model { Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD) } else { Style::default() };
        ...
        Mode::SessionSelect => {
             ...
             let items: Vec<ListItem> = app.available_sessions.iter().map(|s| {
                 let style = if s == &app.current_session {
                     Style::default().fg(Color::Green).add_modifier(Modifier::BOLD).add_modifier(Modifier::ITALIC)
                 } else {
                     Style::default()
                 };
                 let text = if s == &app.current_session { format!("{} (current)", s) } else { s.clone() };
    ...
    for line in &text.lines {
        ...
        if line_width == 0 {
            height += 1;
             ...
             if height == 0 && line_width > 0 { height = 1; }
        }
    }
    height
}